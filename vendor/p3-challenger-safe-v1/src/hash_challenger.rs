    ...
    H: CryptographicHasher<T, [T; OUT_LEN]>,
{
    pub fn new(initial_state: Vec<T>, hasher: H) -> Self {
        Self { input_buffer: initial_state, output_buffer: vec![], hasher }
    }
...
mod tests {
    ...
    impl CryptographicHasher<F, [F; OUT_LEN]> for TestHasher {
            ...
            I: IntoIterator<Item = F>,
        {
            let (sum, len) = input
                .into_iter()
                .fold((F::zero(), 0_usize), |(acc_sum, acc_len), f| (acc_sum + f, acc_len + 1));
        ...
        {
            let (sum, len) =
                input.into_iter().fold((F::zero(), 0_usize), |(acc_sum, acc_len), n| {
                    (acc_sum + n.iter().fold(F::zero(), |acc, f| acc + *f), acc_len + n.len())
                });
            [sum, F::from_canonical_usize(len)]
        }
    }
    ...
    fn test_hash_challenger() {
        ...
        assert_eq!(hash_challenger.input_buffer, vec![expected_sum, expected_len]);
        assert_eq!(hash_challenger.output_buffer, vec![expected_sum, expected_len]);

        let new_element = F::from_canonical_u8(11);
        hash_challenger.observe(new_element);
        assert_eq!(hash_challenger.input_buffer, vec![expected_sum, expected_len, new_element]);
        assert_eq!(hash_challenger.output_buffer, vec![]);
        ...
        assert_eq!(hash_challenger.output_buffer, [F::from_canonical_u8(new_expected_sum)])
    }
}
