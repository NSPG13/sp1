    ...
    + Debug
{
    ...
    fn shifted_powers(&self, start: Self) -> Powers<Self> {
        Powers { base: self.clone(), current: start }
    }
    ...
    fn shifted_powers_packed<P: PackedField<Scalar = Self>>(
        ...
        PackedPowers { multiplier: P::from_f(self.clone()).exp_u64(P::WIDTH as u64), current }
    }
    ...
    + DeserializeOwned
{
    ...
    fn halve(&self) -> Self {
        let half =
            Self::two().try_inverse().expect("Cannot divide by 2 in fields with characteristic 2");
        *self * half
    }
